...
export async function buildDepegEvidence(policy: PolicyTerms, lookbackSeconds = 30 * 60): Promise<EvidenceReport> {
  ...
  const hasContinuousCoverage =
    below.length > 1 &&
    below.every((sample, index) => index === 0 || sample.timestamp - below[index - 1].timestamp <= 90);
  const belowDuration = hasContinuousCoverage
    ? Math.max(0, (below[below.length - 1].timestamp - below[0].timestamp) / 60)
    : 0;
  const report: EvidenceReport = {
    ...
    sellVolumeMultipleBps: data.samples.length
      ? Math.round((calculateSwapVolumeUsd(data) / Math.max(1, Number(data.pool.totalValueLockedUSD))) * 10_000)
      : 0,
    evidence: [
